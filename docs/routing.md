...
```txt
设计/HTML 来源 → Next.js 路由文件 → 浏览器访问 URL → sitemap/导航/Footer/内链
```

规则：

- Stitch 或 HTML 交付包里的页面文件，必须映射到明确的 `app/.../page.tsx`。
- Next.js App Router 的文件夹结构决定最终 URL。
- URL 命名必须先考虑用户理解和 Google 理解，避免临时命名。
- 新增页面时，必须同步检查 `app/sitemap.ts`、导航、Footer、页面内链和 SEO 文档。
- 不允许只新增页面文件但不更新本路由映射表。
